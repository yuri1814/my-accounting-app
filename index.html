...
// IMPORTANT: Replace this with your own Firebase config object from your project settings
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubs = collectionsToFetch.map(({ name, setter }) => {
        const collPath = `artifacts/${appId}/users/${userId}/${name}`;
  ...
  const handleAdd = async (collectionName, data) => {
    if (!userId) return;
    try {
        const collPath = `artifacts/${appId}/users/${userId}/${collectionName}`;
  ...
  const handleUpdate = async (collectionName, docId, data) => {
    if (!userId) return;
    try {
      const docPath = `artifacts/${appId}/users/${userId}/${collectionName}/${docId}`;
  ...
  const handleAddExpense = async (expenseData) => {
      ...
      if(expenseData.installmentId) {
          ...
          if (installment) {
              const installmentRef = doc(db, `artifacts/${appId}/users/${userId}/installments`, expenseData.installmentId);
      ...
      if(expenseData.recurringId) {
          const recurringRef = doc(db, `artifacts/${appId}/users/${userId}/recurring`, expenseData.recurringId);
  ...
  const handleTransfer = async (transferData) => {
      ...
      // 1. Create expense
      const expenseRef = doc(collection(db, `artifacts/${appId}/users/${userId}/expenses`));
      ...
      // 2. Create income
      const incomeRef = doc(collection(db, `artifacts/${appId}/users/${userId}/incomes`));
       batch.set(incomeRef, {
        amount: transferData.amount,
  ...
  return (
    <div className="bg-gray-50 font-sans antialiased" style={{ fontFamily: "'Inter', sans-serif" }}>
